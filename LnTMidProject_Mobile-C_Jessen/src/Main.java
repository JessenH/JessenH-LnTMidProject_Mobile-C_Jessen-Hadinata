		...
		do {
			...
			case 1:
				...
				if (type.equals("Car")) {
					...
					} while (!typeCar.equals("Automatic") && !typeCar.equals("Manual"));
			...
			case 2:
				...
				if (choice !=0) {
					...
					if (kendaraanList.get(choice-1).getType().equals("Car")) {
						typeChoice = kendaraanList.get(choice-1).getTypeCar();			
						mobilInput.nyala(typeChoice, nameChoice);
					}else {
						
						int price = 0, priceTotal = 0;
						...
						priceTotal = (price * kendaraanList.get(choice-1).getWheel());
